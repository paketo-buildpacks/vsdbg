...
func testReusingLayerRebuild(t *testing.T, context spec.G, it spec.S) {
	...
	context("when an app is rebuilt and does not change", func() {
		it("reuses a layer from a previous build", func() {
			...
			Expect(firstImage.Buildpacks[0].Layers).To(HaveKey("vsdbg"))
			...
			Expect(secondImage.Buildpacks[0].Layers).To(HaveKey("vsdbg"))

			Expect(logs).To(ContainLines(
				MatchRegexp(fmt.Sprintf(`%s \d+\.\d+\.\d+`, buildpackInfo.Buildpack.Name)),
			))
			Expect(logs).To(ContainLines(
				MatchRegexp(fmt.Sprintf("  Reusing cached layer /layers/%s/vsdbg", strings.ReplaceAll(buildpackInfo.Buildpack.ID, "/", "_"))),
			))
